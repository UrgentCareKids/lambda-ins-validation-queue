...
def get_patient_details(queue_id, patient_id):
    ...
    cur = _targetconnection.cursor()
    print('got here')
    select_query = f"select mtfd.primary_ins_id, ic.ext_id from mat_tmp_fast_demographics mtfd left join public.ins_cx ic on primary_ins_id = ic.pri_ins_id where mtfd.pond_id = '{patient_id}' and mtfd.primary_ins_id ilike ic.pri_ins_id  and ic.ext_source = 'WAVE'"
    cur.execute(select_query,)
    ins_id = cur.fetchall()
